...
const header_nav = document.querySelector('#js-header-nav')
const nav_anchor = header_nav.querySelectorAll("a[href*='#']")
...
function nav_open() {
    ...
    header_nav.classList.add('is-open');
    header_nav.classList.remove('is-sp-close');
}

function nav_close() {
    sp_button.classList.remove('is-open');
    header_nav.classList.remove('is-open');
    header_nav.addEventListener('animationend', () => {
        header_nav.classList.add('is-sp-close');
    }, {once: true})
}
...
window.AddPackages.SmoothScroll('a[href*="#"]',{
...
triggerElements.forEach(function (triggerElement){
    ...
    window.AddPackages.SimpleScrollTrigger({
        // トリガー要素。必須。
